...
3. **Set the workflow title if empty:**
   ...
   ```
4. Close with the story workflow's native structured question/fallback format:
   ```
   Captured.
   Recommended next:
...
4. **Parse offer** — ONE compact planning line plus a soft next step:
   ...
   If N>0, V>0, M>0, or S>0, ask with the story workflow's native structured question/fallback format:
   ```
   Recommended next:
