...
Calls go via `--stage` — see the project `AGENTS.md` § "Draft gate".
...
rendered), pass `--source-node-id <note_id>` — see the project `AGENTS.md` §
...
Pick the one that fits. For source lookup, follow the project `AGENTS.md` § "Choosing context"; this skill only owns video-specific prompt and CLI shape.
...
One sentence with the price — see the project `AGENTS.md` § "Draft gate". `--ref-source-id` flags drive provenance edges; they're captured in the draft argv and materialize on the real `video_result` after the user fires.

## On failure

Shape and class taxonomy: see the project `AGENTS.md` § "Failure handling". Video-specific:
