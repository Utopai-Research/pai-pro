...
Pick the one that fits. For target lookup, follow the project `AGENTS.md` § "Choosing context"; this skill only owns voice-specific prompt and CLI shape.
...
- Run via Bash (`$PAI_REPO_ROOT` is exported by the viewer — see the project `AGENTS.md` § "Media CLIs / Invocation path"):
  ```
...
- `text` template — a short in-character line the user will actually hear. 1–3 sentences, ≤200 characters is plenty. Pick something that reveals the character:
  ...
  - or a catchphrase.
- Calls go via `--stage` — see the project `AGENTS.md` § "Draft gate". Bulk asks: one Bash call per target in a single turn, each becoming its own draft card.
...
See the project `AGENTS.md` § "Failure handling". For `content_filtered`, drop charged adjectives in the prompt.
