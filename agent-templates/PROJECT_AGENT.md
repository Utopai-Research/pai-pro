...
If your shell/tool returns control while the command is still waiting for review, reply in one short sentence naming the price (*"Staged a 10s 1080p clip — $3.41."*). Don't paste the JSON; don't repeat the prompt; don't promise a result until the final JSON line arrives.
...
- `cancelled` — the user discarded the draft; don't retry unless they ask.
