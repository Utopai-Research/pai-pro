...
- **一个故事媒体 API 服务**：统一处理图像、视频、声音和 4K 放大，因此制作流程不需要为每一个模型分别接入不同供应商。([API 服务详情](docs/api_service.md))
